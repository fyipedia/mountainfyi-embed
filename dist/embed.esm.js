/* mountainfyi-embed v1.0.0 | MIT | https://widget.mountainfyi.com */
...
// src/inline/elevation-badge.ts
function renderElevationBadge(data, _config) {
  var _a, _b;
  const meters = Number((_b = (_a = data.elevation) != null ? _a : data.value) != null ? _b : 0);
  return `<span class="geofyi-badge">${formatElevation(meters)}</span>`;
}

// src/inline/difficulty-badge.ts
function renderDifficultyBadge(data, _config) {
  var _a, _b, _c;
  const grade = Number((_c = (_b = (_a = data.difficulty_grade) != null ? _a : data.grade) != null ? _b : data.value) != null ? _c : 0);
  const info = classifyDifficulty(grade);
  return `<span class="geofyi-badge" style="background:${info.bgColor};color:${info.textColor}">Grade ${grade}: ${info.label}</span>`;
}

// src/_entry_mountainfyi.ts
async function processElement(el, config) {
  ...
  switch (type) {
    ...
    case "search":
      ...
      break;
    case "elevation-badge": {
      const shadow = createShadow(el, config);
      const root = createWidgetRoot(shadow, el, "geofyi-inline");
      const data = {};
      for (const [k, v] of Object.entries(el.dataset)) {
        data[k] = v;
      }
      root.innerHTML = renderElevationBadge(data, config);
      break;
    }
    case "difficulty-badge": {
      ...
      root.innerHTML = renderDifficultyBadge(data, config);
      break;
    }
    default:
      console.warn(`[mountainfyi-embed] Unknown widget type: ${type}`);
  }
}
...
(function bootstrap() {
  const config = '{"site":"mountainfyi","name":"MountainFYI","domain":"mountainfyi.com","accent":"#57534E","attribute":"data-mountainfyi","apiBase":"https://mountainfyi.com/api/v1/","searchPath":"/search/","entityName":"Mountains","entitySlug":"mountains"}';
...
function makeWidgetElement(widgetType, initFn, domainAttrs) {
  ...
  return class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._syncDataAttrs();
      initFn(this, '{"site":"mountainfyi","name":"MountainFYI","domain":"mountainfyi.com","accent":"#57534E","attribute":"data-mountainfyi","apiBase":"https://mountainfyi.com/api/v1/","searchPath":"/search/","entityName":"Mountains","entitySlug":"mountains"}');
    }
    ...
    _syncDataAttrs() {
      const attrKey = '{"site":"mountainfyi","name":"MountainFYI","domain":"mountainfyi.com","accent":"#57534E","attribute":"data-mountainfyi","apiBase":"https://mountainfyi.com/api/v1/","searchPath":"/search/","entityName":"Mountains","entitySlug":"mountains"}'.attribute.replace("data-", "");
...
(function registerElements() {
  ...
  const site = '{"site":"mountainfyi","name":"MountainFYI","domain":"mountainfyi.com","accent":"#57534E","attribute":"data-mountainfyi","apiBase":"https://mountainfyi.com/api/v1/","searchPath":"/search/","entityName":"Mountains","entitySlug":"mountains"}'.site;
  const defs = [
